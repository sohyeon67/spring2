...
public class InterceptorController {
	 ...
	 * 			(작성하면서 어떤 방법이 있는지 머리속에 떠오르길 바랍니다!!)
	 * 
	 ...
	 * 			어떤 수정을 가하거나 코드 중복 없이 이슈를 해결할 수 있을 것입니다. 그러나 이 방법은 한가지 단점이 있습니다.
	 * 			바로 Filter가 DispatcherServlet보다 먼저 객체가 생성되고 스프링 컨테이너 밖에 존재하기 때문에 컨테이너를 통한
	 * 			DI를 받을 수 없다는 점입니다. 물론 아예 불가능 하지는 않습니다. 설정이나 이런 부분이 복잡하고 귀찮을 뿐이죠...
	 ...
	 * 			스프링 빈에게 필터링을 위임하는 방법도 몇 가지 불편한 점들이 있습니다.
	 * 			반드시 위임을 받을 빈은 Filter를 구현하고 있어야 하고, root-context.xml에서 관리되는 빈이어야만 한다는 것입니다.
	 * 			
	 * 			두번째로 AOP방법론에 다른 공통 기능을 정의한 Advice를 구현하고, pointcut을 통해 적절한 target 컨트롤러를 골라낸 다음
	 * 			두 설정으로 Aspect를 생성해야 런타임에 컨트롤러와 위빙하도록 하는 방법을 생각해볼 수 있습니다. 실제 보안 프레임워크들에서도
	 ...
	 * 
	 * 			preHandle() 메소드는 컨트롤러 객체를 실행하기 전에 필요한 기능을 구현할 때 사용되며, handler 파라미터는 웹
	 * 			요청을 처리할 컨트롤러 객체입니다. 이 메소드를 사용하면 컨트롤러를 실행하기 전에 컨트롤러에서 필요로 하는 정보를 생성하거나
	 * 			접근 권한이 없는 경우 리턴값으로 false를 반환하여 컨트롤러가 실행되지 않도록 하는 작업이 가능합니다.
	 ...
	 * 			실행 시간을 기록하는 등의 후처리를 하기에 적합한 메소드입니다.
	 * 
	 * 			정리하자면 HandlerInterceptor는 세 가지 메소드를 통해 AOP 방법론에서 시점에 따른 여러 advice들의 역할을
	 * 			하나의 handlerInterceptor객체가 전담할 수 있는 구조를 갖고 있습니다!!
	 * 
	 * 		3. 인터셉터 설정
	 * 	
