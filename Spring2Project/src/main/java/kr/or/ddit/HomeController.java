...
public class HomeController {
	 ...
	 * 			@ResponseBody 대신에 @RestController를 이용하여 대체할 수 있습니다. (클래스 라인에 @RestController)
	 */
	@ResponseBody
	...
	public Member goHome0301() {
		 ...
		 * @ResponseBody를 설정하지 않고 객체를 응답으로 내보낼 시, 데이터로 인식되는게 아닌 페이지로 인식되므로 404 에러가 발생한다.
		 */
	...
	public ResponseEntity<byte[]> goHome1102() throws IOException {
		...
		try {
			in = new FileInputStream("C:\\Users\\PC-03\\Pictures\\Saved Pictures\\planet.jpg");
			// MediaType.APPLICATION_OCTET_STREAM은 이진 파일을 위한 기본값입니다. (다운로드를 진행할 때 미디어 타입)
