...
public class SecurityController {
	 ...
	 * 			# 시큐리티 인증 구조
	 * 
	 * 				클라이언트에서 타겟으로 들어가기 위해서 요청을 진행합니다. 이때 타겟에 설정되어 있는 요청 접근 권한이 '사용자'
	 ...
	 * 				UserDetailService에서 넘겨받은 Authentication 객체 정보를 이용해서 Database에 일치하는 회원정보를
	 * 				조회하여 꺼낸다. 꺼낸 정보를 UserDetails로 만들고 최종 User객체에 회원정보를 등록한다.
	 * 				등록이 되면서 User Session 정보가 생성된다. 이후 스프링 시큐리티 내 SecurityContextHolder
	 * 				(시큐리티 인메모리)에 UserDetail정보를 저장한다. 그리고 최종적으로 JSESSIONID가 유효한지를 검증 후
	 ...
	 * 			- 스프링 시큐리티 5버전부터는 패스워드 암호화 처리기를 반드시 이용하도록 변경이 되었습니다.
	 ...
	 * 					** 필요에 의한 패키지 라인을 bean등록하여 사용해야할 때 스프링 웹 설정에서 base-package를 설정할 수 있다.
	 * 
	 ...
	 * 					공지사항 게시판 등록 페이지는 권리자 권한으로만 접근 가능하므로 접근이 거부된다.
	 ...
	 * 				실행되고 response 내장객체를 활용하여 /accessError URL로 이동하여 접근 거부시 보여질 페이지로 이동하지만
	 * 				이곳에서 더 많은 로직을 처리할 수도 있다. (request, response 내장객체를 이용하여 다양한 처리 가능)
	 * 
	 * 				# 화면 설명
	 * 				- 일반 게시판 등록 페이지
	 * 				> 접근 제한에 걸려 스프링 시큐리티가 제공하는 로그인 페이지가 나타나고, 회원 권한을 가진 계정으로 접근시 접근 가능
	 * 
	 * 				- 공지사항 게시판 등록 페이지
	 * 				> 접근 제한에 걸려 스프링 시큐리티가 제공하는 로그인 페이지가 나타나고, 
	 * 				회원 권한을 가진 계정으로 접근 시에 공지사항 게시판 등록 페이지는 관리자 권한만 접근 가능하므로 접근이 거부된다.
	 * 				이때, access-denied-handler로 설정되어 있는 ref 속성에 부여된 클래스 메소드로 이동하고
	 ...
	 * 				# 로그인 성공 처리자 클래스 정의
	 * 				- 로그인 성공 처리자
	 * 				> SavedRequestAwareAuthenticationSuccessHandler는 AuthenticationSuccessHandler의 구현 클래스입니다.
	 * 				인증 전에 접근을 시도한 URL로 리다이렉트하는 기능을 가지고 있으며 스프링 시큐리티에서 기본적으로 사용되는 구현 클래스입니다.
	 * 				- 로그인 성공 처리자2(수업시간에 이거 사용)
	 ...
	 * 				CustomLoginSuccess 클래스로 넘어가 넘겨받은 파라미터들 중 authentication안에
