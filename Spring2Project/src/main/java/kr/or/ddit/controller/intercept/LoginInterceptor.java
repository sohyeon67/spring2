...
public class LoginInterceptor extends HandlerInterceptorAdapter {
	 ...
	 * 		이때, 해당 컨트롤러 메소드가 실행되기 전에 LoginInterceptor의 preHandle이 동작하고 
	 * 		'userInfo' 세션명을 가진 세션정보를 조회한다. 조회된 정보가 존재한다면, 세션을 삭제처리한다.
	 * 		그리고 타겟을 거쳐 다시 인터셉터로 넘어올 때 데이터 전달자가 전달해준 회원정보가 존재한다면 'userInfo'키로
	 * 		회원 정보를 세션에 등록하고 '/'로 리다이렉트한다. 그렇지 않는 경우는 해당 타겟을 정상 실행 후 리턴하는 결과 페이지로 이동한다.
	 */
