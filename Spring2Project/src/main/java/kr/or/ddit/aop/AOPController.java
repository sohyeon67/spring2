...
public class AOPController {
	 ...
	 * 
	 * 			306호 반장 홍길동학생이 신입으로 프로젝트를 진행하고 있었습니다.
	 * 			그러던 어느날, 강감찬 팀장님을 통해서 '반장님, 지금 개발중인 서비스 처리 속도좀 로그로 남겨주세요!'라는 부탁을 받는다.
	 * 			반장 홍길동학생은 부탁받은 요구사항을 이행하기 위해서 본인이 만들고 있는 서비스 로직에서 처리 속도를 찍어볼 메소드를 개발해
	 * 			처리 속도가 잘 찍히는 걸 확인합니다.
	 * 			기분이 좋은 홍길동학생은 팀장님께로 달려갑니다.
	 * 			그리고 강감찬 팀장님께 해당 내용을 컨펌받습니다.
	 * 			강감찬 팀장님은 아주 긍정적인 검토안을 홍길동학생에게 전달하면서 우리 서비스 전체에도 각 처리속도를 찍어주세요라고 다시 전달합니다.
	 * 			홍길동학생은 본인이 만들어 낸 메소드를 각 기능별 서비스로직에 하나하나씩 약 2만개쯤 작성 하던 중, 의문을 갖습니다.
	 * 
	 * 				"서비스 로직에서 제일 중요한 로직은 본래의 기능이 제일 중요하고 지금 내가 작성하고 있는 로직은 옵션(부가기능)이 추가되는게 아닌가?"
	 * 				"그럼 이걸 하나의 묶음으로는 처리가 불가능한건가?"
	 * 
	 * 			하는 생각을 하게 됩니다.
	 * 			여기서, 시간을 측정하고 권한을 체크하는 등의 기능은 옵션과 같은 부가기능으로 일종의 인프라 로직이라고 하는데,
	 * 			이 인프라 로직은 애플리케이션 전 영역에서 나타날 수 있고, 중복 코드를 만들어내 개발의 효율성을 저하시키고 유지보수가
	 * 			힘들어 질 수 있습니다.
	 * 
	 ...
	 * 
	 * 			이러한 횡단 관심사를 통해서 프로그래밍하는 것을 AOP라고 한다.
	 * 
	 * 			# 간단하게 보고 넘어가기 (용어)
	 * 			- Aspect(애스팩트)
	 * 				: AOP의 단위가 되는 횡단 관심사
	 ...
	 * 					- 예외처리 (어노테이션 및 xml파일 등을 활용한 예외처리)
	 * 
	 * 				1-4) AOP 관련 용어
	 * 
	 ...
	 * 						: 어디에 적용할 것인지 결정, 메소드/객체 생성시/필드접근 등등
	 * 					- 어드바이스(Advice)
	...
	public void startLog(JoinPoint jp) {
		...
		// getSignature() : 어떤 클래스의 어떤 메소드가 실행되었는지를 보여준다. (파라미터 타입은 무엇인지 보여줌)
	 ...
	 * 		: around 어드바이스에서 사용함
	 * 
	 * 		스프링프레임워크가 컨트롤 하고 있는 비즈니스로직 호출을 가로챈다.
